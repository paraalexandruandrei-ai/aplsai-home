...
def init_operations(app, app_module):
    """Registra stato operativo e audit senza modificare le tabelle legacy.

    Le nuove tabelle vengono create separatamente: questo evita migrazioni distruttive
    durante la transizione dal prototipo alla V1 operativa.
    """
    db = app_module.db
    ...
    def staff_operations():
        ...
        for cp in app_module.ClientProfile.query.all():
            ...
            if not op:
                ...
                db.session.add(op)
                db.session.flush()
            client = app_module.client_obj(cp.user_id)
            rows.append({"client": client, "operation": op.to_dict()})
        db.session.commit()
    ...
    def update_client_operation(client_id):
        ...
        financial_state = data.get("financial_state", op.financial_state)
        next_action = str(data.get("next_action", op.next_action) or "").strip()[:255]
        assigned_to = str(data.get("assigned_to", op.assigned_to) or "").strip()[:160]
        blocked_reason = str(data.get("blocked_reason", op.blocked_reason) or "").strip()[:500]

        if phase not in PHASES:
            return jsonify(error="Fase operativa non valida."), 400
        if financial_state not in FINANCIAL_STATES:
        ...
        try:
            due_at = parse_iso_datetime(data.get("next_action_due_at")) if "next_action_due_at" in data else op.next_action_due_at
        except (ValueError, TypeError):
            return jsonify(error="Scadenza prossima azione non valida."), 400

        previous_financial = op.financial_state
        op.phase = phase
        op.financial_state = financial_state
        op.next_action = next_action
        op.next_action_due_at = due_at
        op.assigned_to = assigned_to
        op.blocked_reason = blocked_reason
        op.updated_at = utcnow()
        if financial_state != previous_financial and financial_state in {"capitale_verificato", "mutuo_deliberato", "pre_delibera"}:
            op.financial_verified_at = utcnow()

        audit(u, "client_operation_update", "client", client_id, f"phase={phase}; financial_state={financial_state}")
        db.session.commit()
